...
function AccountPanel(): React.ReactElement {
  ...
  // Inline sign-in form state (shown in AccountPanel when !userToken)
  ...
  const handleSignInSubmit = async (): Promise<void> => {
    if (!profile || !siUrl || !siEmail || !siPw) return;
    setSiBusy(true);
    setSiError('');
    try {
      await loginAndCacheHash(siUrl.trim(), siEmail.trim(), siPw, profile.id);
      setSiPw('');
      setSigningIn(false);
    } catch (e) {
      setSiError(e instanceof Error ? e.message : String(e));
    } finally {
      setSiBusy(false);
    }
  };
        ...
        <>
          ...
          <div className="rounded-lg border border-border p-3 space-y-1">
            ...
            {userToken ? (
              ...
              <div className="space-y-3 pt-1">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="url"
                    value={siUrl}
                    onChange={(e) => setSiUrl(e.target.value)}
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') void handleSignInSubmit(); }}
                    disabled={siBusy}
                  />
                </div>
                {siError && <p className="text-xs text-destructive">{siError}</p>}
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    onClick={handleSignInSubmit}
                    disabled={siBusy || !siUrl || !siEmail || !siPw}
                  >
              ...
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">
                  Not signed in. Sign in to connect this profile to a SyncroLWS server.
                </p>
          ...
          {/* ── Require enterprise password at login ─────────────────── */}
            ...
            <div className="rounded-lg border border-border p-3">
              <label className="flex items-start gap-3 cursor-pointer select-none">
                <input
                  ...
                  checked={!!profile.useEnterprisePwAtLogin}
                  onChange={(e) =>
                    profile && updateProfile(profile.id, { useEnterprisePwAtLogin: e.target.checked })
                  }
                />
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    When enabled, the ProfileGate always asks for your enterprise password
                    to enter this profile — even when a valid session token is cached.
                  </p>
                </div>
              </label>
            </div>
